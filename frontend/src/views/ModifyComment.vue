<template>
    ...
    <main class="block_modify_comment">
        <h1 class="comment_title_modify">Modifie ton commentaire</h1>
        <!-- section modification de commentaire -->
        <form v-on:submit.prevent class="form_comment_modify">
            <textarea v-model="text" class="comment_text_modify" aria-label="Zone de texte commentaire à modifier">{{this.text}}</textarea>
            <button class="button_modifyComment" v-on:click.preventDefault="ModifyComment()">Modifier</button>
        </form>
    </main>
</template>
